...
**Excel workbooks** with several sheets: the importer scores the first rows of each sheet for BOM-like headers (e.g. Value, Designator, R1…) and uses the best-scoring sheet instead of always the first tab—so a cover sheet or empty first tab does not hide the BOM. If the BOM table starts below title rows, the importer **detects the header row** (REF, Value, MFR, …) and trims rows above it.

Example BOM files used for regression checks live in **`tests/exampleBOM/`** (CSV and Excel).
